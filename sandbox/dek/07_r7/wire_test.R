...
#'
#' @author Dan Kelley
wire_test <- function(model = "1/4in wire/jack", buoyancyPerMeter = NULL, areaPerMeter = NULL, CD = NULL, length = NULL) {
    ...
    wire(model = model, buoyancy = length * buoyancyPerMeter, height = length, area = length * areaPerMeter, CD = CD, source = source, originalName = originalName)
} # wire()
