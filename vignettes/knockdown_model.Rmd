...
The `buoyancy` is in kg, the next three quantities are in cm, and `CD` is
unitless. The meaning of the last column is unclear.

```{r echo=FALSE}
floats <- read.csv("../create_data/floats_dewey.csv")
knitr::kable(floats)
```

## Wire characteristics

The `buoyancy` is in kg/m, the next three quantities are in cm, and `CD` is
unitless. The meaning of the last column is unclear. In this category,
`length_height` should be ignored.

```{r echo=FALSE}
wires <- read.csv("../create_data/wires_dewey.csv")
knitr::kable(wires)
```
...
```{r echo=FALSE}
chains <- read.csv("../create_data/chains_dewey.csv")
knitr::kable(chains)
```
