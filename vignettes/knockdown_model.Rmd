---
title: Two-dimensional Mooring Model
author: Dan Kelley
date: 2021 May 15
...
* Drag coefficient, $C_{Di}$, defined as the ratio of drag force, ${\cal D}_i$, to
  $\frac{1}{2}A_i\rho_i u_i|u_i|$.

Tension is computed between each element, with ${\cal T}_i$ representing the value
above the $i$-th element (pulling upward, for case with no water motion) and
${\cal T}_{i+1}$ representing the value below it.
...
\begin{equation}
{\cal D}_i = \frac{1}{2} A_i C_{Di} \rho_i u_i |u_i|
\end{equation}
acts only in the $x$ direction, for a horizontal current.

Note that ${\cal D}_i>0$ for a force in the positive $x$ direction, and ${\cal
B}_i>0$ for a force in the positive $z$ direction (upward). For positive
velocities, ${\cal D}_i$  is non-negative, irrespective of the properties of
element $i$.  However, the buoyancy force, ${\cal B}_i$, may be positive or
negative, depending on the difference in density between the element and
seawater.
...
\label{e:balance_1}
\begin{split}
{\cal D}_1 &= {\cal T}_2 \sin\phi_2\cr
{\cal B}_1 &= {\cal T}_2 \cos\phi_2
\end{split}
\end{equation}
...
\label{e:compute_2}
\begin{split}
\phi_2 &= \tan^{-1}\frac{{\cal D}_1}{{\cal B}_1}\cr
{\cal T}_2 &= \sqrt{{\cal D}_1^2+{\cal B}_1^2}
\end{split}
\end{equation}
...
\label{e:balance_2}
\begin{split}
{\cal D}_2 + {\cal T}_2 \sin\phi_2 &= {\cal T}_3 \sin\phi_3\cr
{\cal B}_2 + {\cal T}_2 \cos\phi_2 &= {\cal T}_3 \cos\phi_3
\end{split}
\end{equation}
with which
\begin{equation}
\label{e:compute_3}
\begin{split}
\phi_3 &= \tan^{-1}\frac{{\cal D}_2+{\cal T}_2\sin\phi_2}{{\cal B}_2+{\cal T}_2\cos\phi_2}\cr
{\cal T}_3 &= \sqrt{({\cal D}_2+{\cal T}_2\sin\phi_2)^2+({\cal B}_2+{\cal T}_2\cos\phi_2)^2}
\end{split}
\end{equation}
may be calculated, since $\phi_2$ and ${\cal T}_2$ are known from \eqref{e:compute_2}.
...
\label{e:compute_i}
\begin{split}
\phi_i &= \tan^{-1}\frac{{\cal D}_{i-1}+{\cal T}_{i-1}\sin\phi_{i-1}}{{\cal B}_{i-1}+{\cal T}_{i-1}\cos\phi_{i-1}}\cr
{\cal T}_i &= \sqrt{({\cal D}_{i-1}+{\cal T}_{i-1}\sin\phi_{i-1})^2+({\cal B}_{i-1}+{\cal T}_{i-1}\cos\phi_{i-1})^2}
\end{split}
\end{equation}
for $i=2,\dots,n$.
...
1. Use \eqref{e:compute_2} to compute $\phi_2$ and ${\cal T}_2$.
2. Use \eqref{e:compute_i} to compute $\phi_i$ and ${\cal T}_i$ sequentially,
   for $i=3$ to $i=n$.

If the current is depth-variable, then the drag values ${\cal D}_i$ depend on
...
\autoref{f:example_01} shows the result of simulating a *Hydro Float 20*
float connected to the bottom with a *Mooring Systems 3x19 3/16"* wire cable,
in a region with a depth-independent 2m/s current, which might be encountered
in a region with strong tides.  Note that the float is predicted to sink
$9.33$m with this current.  However, the predicted knockdown is only $0.72$m
with a $1$m/s current.  This difference provides an indication of the
importance of designing a mooring for the system under study.

![Response of a mooring consisting of a *Hydro Float 20* atop 100m of *Mooring Systems 3x19 3/16"* wire cable, in a 1m/s current. **Left**: Cable tension, in kg, with gray for the $u=0$ case and black for $u=1$m/s. **Right:** Mooring shape, with gray and black as in the left panel.\label{f:example_01}](example_01.pdf)
...
\begin{equation}
{\cal T}_{i+1} = {\cal T}_i + {\cal B}_i
\end{equation}
which yields
\begin{equation}
{\cal T}_i = \sum_{1}^{i-1} {\cal B}_j
\end{equation}
...
bottom tension, ${\cal T}_n$, may be interpreted as the minimum anchor weight that
