...
The buoyancy force for a solid object is found with (Equation 1)
\begin{equation}
\B_i = -g m_i
\end{equation}
and for a portion of a cable of length $\Delta l$ with (Equation 2)
\begin{equation}
\B_i = -g \mu_i \Delta l
\end{equation}
...
\end{split}
\end{equation}
which permit computation of (Equation 5)
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
may be calculated, since $\phi_2$ and $\T_2$ are known from Equation 5.

This reasoning applies for all elements below the first one, so that
Equation 7 may be generalized to (Equation 8)
\begin{equation}
\begin{split}
...
1. Use Equation 5 to compute $\phi_2$ and $\T_2$.
2. Use Equation 8 to compute $\phi_i$ and $\T_i$ sequentially,
   for $i=3$ to $i=n$.
