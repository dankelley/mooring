...
draw(mdk, "knockdown", xlim = c(0, 4.0), showDetails = TRUE)
draw(mdk, "velocity")
```
...
```{r}
names <- sapply(md@elements, \(e) e@model)
...
for (i in seq_len(N)) {
    ...
    dz[i] <- k@elements[[iADCP]]@z - md@elements[[iADCP]]@z
}
plot(U, dz, lwd = 2, type = "l", xlab = "Velocity [m/s]", ylab = "Knockdown [m]")
grid()
mtext(sprintf("Knockdown for ADCP nominally at %.0f m depth", -md@elements[[iADCP]]@z))
```
...
mdDraggy <- segmentize(mDraggy, by = 5)
layout(matrix(1:2, nrow = 1), widths = c(0.75, 0.25))
mdkDraggy <- knockdown(mdDraggy, u = u)
draw(mdkDraggy, "knockdown", xlim = c(0, 8.0), showDetails = TRUE)
draw(mdkDraggy, "velocity")
```
