...
```{r}
md <- segmentize(m) # breaks wire portions into approx. 1-m segments
```
...
The plot shown above is quite primitive, so the package offers a `draw()`
function.  For example, this gives a view of tension and shape.
```{r, fig.cap="**Mooring shape and tension profiles created with draw(). The red dashed line on the right-hand panel represents the anchor weight.**", fig.align="center"}
par(mfrow = c(1, 2))
draw(mdk, which = "shape")
...
par(mfrow = c(1, 2))
draw(mdk, which = "shape", fancy = TRUE)
draw(mdk, which = "tension", fancy = TRUE)
```
...
bottom anchor. How will knockdown vary with current strenth?  To answer this,
we might constract a graph as follows.  Similar graphs could be done with
