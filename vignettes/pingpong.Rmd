...
l <- L * cos(phi)
T <- B / cos(phi)
sprintf("R=%.3f m, L=%.3f m, u=%g m/s\n", R, L, u)
sprintf("  D=%.3f N (%.3f kg), B=%.3f N (%.3f kg)\n", D, D/g, B, B/g)
sprintf("  T=%.3f N (%.3f kg), phi=%.1f deg\n", T, T/g, phi*180/pi)
sprintf("  x=%.3f m, l=%.3f m, L-l=%.3f m\n", x, l, L-l)
...
1. The horizontal shift of the float, $x'$.  Here, a $`r u`$ m/s current is
   shown to yield a displacement of $x'=`r sprintf("%.1f",xcm)`$ cm.
2. The vertical float displacement over the zero-velocity case, $z'$. Here,
   this is $`r sprintf("%.1f",zcm)`$ cm.
3. The tension in the line, $T$.  Here, this is $`r round(T,3)`$ Newtons,
   equivalent to `r round(1000*T/g,1)` g.

# A possible current meter based on these ideas

All three of these things could be measured in a classroom or laboratory
