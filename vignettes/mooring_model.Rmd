...
\begin{equation}
\tag{8}
\D_2 + \T_1 \sin\phi_1 = \T_2 \sin\phi_2
\end{equation}
and
\begin{equation}
\tag{9}
...
for $\T_2$ and $\phi_2$, using
\begin{equation}
\tag{10}
\T_2 = \sqrt{(\D_2+\T_1\sin\phi_1)^2 + (\B_2+\T_1\cos\phi_1)^2}
\end{equation}
and
\begin{equation}
\tag{11}
...
\begin{equation}
\tag{12}
\T_i = \sqrt{(\D_i+\T_{i-1}\sin\phi_{i-1})^2+(\B_i+\T_{i-1}\cos\phi_{i-1})^2}
\end{equation}
and
\begin{equation}
\tag{13}
...
1. First, use Equations 4 through 7 to compute $\T_1$ and $\phi_1$, the angle and
   tension below the top ($i=1)$ element.
2. Use Equations 12 and 13 to compute $\T_i$ and $\phi_i$ for $i=2$.
