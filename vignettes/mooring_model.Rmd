...
* Buoyancy 'force' expressed in kilograms, i.e. the actual force in Newtons,
  divided by $g$, the acceleration due to gravity.
  For a solid object such as a float or instrument, this is a simple number,
  denoted $m_i$ below. For a wire or chain, it is expressed per unit length of
  cable, i.e. in kg/m, and denoted $\mu_i$ below.  Manufacturers tend to
  provide $m$ or $\mu$ values in technical documentation about mooring
  components. Note that they refer to in-water buoyancy force, but the
  distinction between freshwater and seawater is seldom made clear.
...
\begin{equation}
\begin{split}
\D_1 &= \T_1 \sin\phi_1\cr
\B_1 &= \T_1 \cos\phi_1
\end{split}
\end{equation}
where $\T_1$ is the tension between this element and the one below it, and
...
known, these two equations may be solved for $\T_1$ and $\phi_1$, yielding
(Equation 5)
\begin{equation}
\begin{split}
\T_1 &= \sqrt{\D_1^2 + \B_1^2}\cr
\phi_1 &= \tan^{-1}\frac{\D_1}{\B_1}
\end{split}
\end{equation}

If there are more than 2 elements in the mooring, this same reasoning may be extended
to the next element, although now there is a tension force pulling upward. This situation is described by (Equation 6)
\begin{equation}
\begin{split}
\D_2 + \T_1 \sin\phi_1 &= \T_2 \sin\phi_2\cr
\B_2 + \T_1 \cos\phi_1 &= \T_2 \cos\phi_2
\end{split}
\end{equation}

Since $\phi_1$ and $\T_1$ are known from Equation 5, Equation 6 may be solved
for $\T_2$ and $\phi_2$, using (Equation 7)
\begin{equation}
\begin{split}
\T_2 &= \sqrt{(\D_2+\T_1\sin\phi_1)^2 + (\B_2+\T_1\cos\phi_1)^2}\cr
\phi_2 &= \tan^{-1}\frac{\D_2+\T_1\sin\phi_1}{\B_2+\T_1\cos\phi_1}
\end{split}
\end{equation}
...
\begin{equation}
\begin{split}
\T_i &= \sqrt{(\D_i+\T_{i-1}\sin\phi_{i-1})^2+(\B_i+\T_{i-1}\cos\phi_{i-1})^2}\cr
\phi_i &= \tan^{-1}\frac{\D_i+\T_{i-1}\sin\phi_{i-1}}{\B_i+\T_{i-1}\cos\phi_{i-1}}
\end{split}
\end{equation}
applying for elements $i=2,\dots,n-1$.  The limit ends at $n-1$ because is no
tension below the bottom element, so neither $\T_n$ nor $\phi_n$ can be
defined.
...
$n$:
1. First, use Equation 5 to compute $\T_1$ and $\phi_1$, the angle and tension below
   the top ($i=1)$ element.
2. Then, use Equation 8 to compute $\T_i$ and $\phi_i$ for $i=2$, and then use
   it again to compute these things at $i=3$, etc., down to $i=n-1$.
